...
var colors = {
    selected: 'rgba(255,199,0,.6)',
    vertex: 'rgba(256,0,236,1)',
...
$(function () {
    ...
    $('#button_import_cancel').click(function () {
        var detect_desn = confirm('Do you want to return to visual mode without saving changes?');
        if (detect_desn) {
            hide_box();
            czyTekst = false;
        } else return false;
    ...
    $('#button_import_select_all').click(function () {
        ...
        for (let joint of stadium.joints)
            if (joint.length == "null") joint.length = null;
        ...
        //console.log("skopiowane");
        console.log(document.getElementById("button_import_select_all").innerHTML);
        document.getElementById("button_import_select_all").innerHTML = "COPIED!";
        setTimeout(function () {
            document.getElementById("button_import_select_all").innerHTML = "<img src=\"./HBSE_files/general/general_all.png\" style=\"height: 12px; width: 12px\"> Copy All";
        }, 2000);
    ...
    $('#button_import_clear').click(function () {
        var detect_desn = confirm('Are you sure?');
        if (detect_desn) $('#textarea_import').val('');
        else return false;
    });

    $('#button_downloadMap').click(function () {
        //////
        for (let joint of stadium.joints)
            if (joint.length == "null") joint.length = null;
        if (stadium.canBeStored == "true" || stadium.canBestored == true) stadium.canBeStored = true;
        else stadium.canBeStored = false;
        var blob = new Blob([JSON.stringify(stadium)], {
            type: 'text'
        });
    ...
    $('#button_import_import').click(function () {
        ...
        try {
            ...
            st = undefined;
            console.log('erro')
            alert('Error, check your script again.');
        }
        if (!st) {
            alert('Error, check your script again.');
            return;
		...
		if (st.discs[0].pos == undefined) st.discs[0].pos = [0,0];
        load(st);
        modified();
        hide_box();
        czyTekst = false;
    ...
    $('#button_redSpawnPoint').click(function () {
        ...
        console.log(xxx, yyy);
        stadium.redSpawnPoints.unshift([Number(xxx), Number(yyy)]);
        render(stadium)
    ...
    $('#button_blueSpawnPoint').click(function () {
        ...
        document.getElementById("prop_spawnPointY").value = "";
        console.log(xxx, yyy);
        stadium.blueSpawnPoints.unshift([Number(xxx), Number(yyy)]);
        render(stadium)
    });
    ...
    $('#button_newTrait').click(function () {
        ...
        Object.defineProperty(stadium.traits, zetName, {
            value: zet,
            enumerable: true
        });
        /*console.log(stadium.traits);
...
function render(st) {
    ...
    */ ///
    ...
    if (settings.preview) {
        ...
        // 미리 보기 그리기
        ctx.beginPath();
        if(prop_bp_radius.value !== "") {
        ctx.arc(0, 0, prop_bp_radius.value, 0, Math.PI * 2, true);
        } else {
            ctx.arc(0, 0, haxball.default_disc_radius, 0, Math.PI * 2, true);
        }
        ctx.fillStyle = `#${prop_bp_color.value}`
        ctx.strokeStyle = 'rgb(0,0,0)';
        ctx.lineWidth = 2;
        ctx.fill();
        ctx.stroke();
        ...
        // 레드 그리기
        ctx.beginPath();
        if (stadium.redSpawnPoints[0] != undefined && prop_pp_radius.value !== "") {
            ctx.arc(stadium.redSpawnPoints[0][0], stadium.redSpawnPoints[0][1], prop_pp_radius.value, 0, Math.PI * 2, true);
        } else if(stadium.redSpawnPoints[0] != undefined && prop_pp_radius.value == ""){
            ctx.arc(stadium.redSpawnPoints[0][0], stadium.redSpawnPoints[0][1], haxball.default_player_radius, 0, Math.PI * 2, true);
        } else {
            ctx.arc(-st.spawnDistance, 0, haxball.default_player_radius, 0, Math.PI * 2, true);
        }
        ctx.fillStyle = '#c2625f';
        ctx.lineWidth = 2.5;
        ctx.fill();
        ctx.stroke();
        ...
        // 블루 그리기
        ctx.beginPath();
        if (stadium.blueSpawnPoints[0] != undefined && prop_pp_radius.value !== "") {
            ctx.arc(stadium.blueSpawnPoints[0][0], stadium.blueSpawnPoints[0][1], prop_pp_radius.value, 0, Math.PI * 2, true);
        } else if(stadium.blueSpawnPoints[0] != undefined && prop_pp_radius.value == ""){
            ctx.arc(stadium.blueSpawnPoints[0][0], stadium.blueSpawnPoints[0][1], haxball.default_player_radius, 0, Math.PI * 2, true);
        } else {
            ctx.arc(st.spawnDistance, 0, haxball.default_player_radius, 0, Math.PI * 2, true);
        }
        ctx.fillStyle = '#5f8bc2';
        ctx.lineWidth = 2.5;
        ctx.fill();
        ctx.stroke();
...
function renderbg(st, ctx) {
    ...
    if (bg.type == 'grass' || bg.type == 'hockey') {
        ...
        ctx.fillStyle = `#${prop_bg_color.value}`;
...
function logout() {
    $.ajax({
        type: 'POST',
        url: 'http://web.archive.org/web/20181129142757/http://haxpuck.com/action/logout',
        data: {
            sessionid: session_id
        }
    });
...
function save(success_continuation) {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://web.archive.org/web/20181129142757/http://haxpuck.com/action/save',
        dataType: 'json',
        data: {
            userid: id,
            sessionid: session_id,
...
function library_query() {
    ...
    var error = function (e) {
        $('<tr></tr>').html(
            $('<td colspan="3"></td>').text('Error Loading Data: ' + e)).
...
function library_edit() {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://web.archive.org/web/20181129142757/http://haxpuck.com/action/stadium',
        dataType: 'json',
        data: {
            sessionid: session_id,
            id: sid
...
function library_delete() {
    ...
    $.ajax({
        type: 'POST',
        url: 'http://web.archive.org/web/20181129142757/http://haxpuck.com/action/delete_hbs',
        dataType: 'json',
        data: {
            sessionid: session_id,
            id: sid
...
function download() {
    save(function (id) {
        ...
        $('<form method="POST">' +
            ...
            'action', 'http://www.naver.com/' + encodeURIComponent(stadium.name) + '.hbs'
